...
import { useSignerHedera } from "@/providers/SignerHederaProvider";

export const SignerHederaView: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  const signer = useSignerHedera();
  ...
  return (
    <DeviceActionsList title="Signer Hedera" deviceActions={deviceActions} />
  );
};
